...
import os, zipfile, subprocess, shutil, json, time, io, socket, sys, signal, atexit

app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "jubayer_hosting_v5_final_pro")
...
app_ports = {}   # (user, app_name) -> port assigned

ADMIN_PASS = os.environ.get("ADMIN_PASS", "5656")

MAIN_FILE_CANDIDATES = ["main.py", "bot.py", "app.py", "index.js", "server.js"]
PIP_INSTALL_TIMEOUT = 420  # seconds
...
# ---------------------------------------------------------------------------
# Security helper: prevent path traversal in file-manager endpoints
# ---------------------------------------------------------------------------

def safe_join(base, *parts):
    """Join paths and guarantee the result stays inside `base`. Returns None if not."""
...
def find_main_file(extract_dir):
    """Search root first, then recurse into subfolders (e.g. backend/main.py).
    If no standard entry filename is found, fall back to the single .py/.js
    file in the project (handles renamed/single-file uploads)."""
    for f in MAIN_FILE_CANDIDATES:
    ...
    # Fallback: if there is exactly one runnable script in the whole project,
    # treat it as the entry file regardless of its name.
    script_matches = []
    for root, dirs, files in os.walk(extract_dir):
        for f in files:
            if f.endswith(".py") or f.endswith(".js"):
                script_matches.append((f, root))
...
def find_requirements(run_dir, extract_dir):
    """Search the whole extracted project recursively for any requirements*.txt file."""
    candidates = []
    for root, dirs, files in os.walk(extract_dir):
        for f in files:
    ...
    if not candidates:
        return None
    for c in candidates:
        if os.path.basename(c).lower() == "requirements.txt":
            return c
    candidates.sort(key=lambda c: len(os.path.relpath(c, run_dir)))
    return candidates[0]


def install_requirements_if_needed(app_dir, run_dir, extract_dir, log_file):
    marker = os.path.join(app_dir, ".deps_installed")
    if os.path.exists(marker):
        return
    req_path = find_requirements(run_dir, extract_dir)
    if req_path:
        log_file.write(f"\n[setup] Installing dependencies from {req_path}...\n")
        log_file.flush()
        try:
            result = subprocess.run(
                [sys.executable, "-m", "pip", "install", "--no-cache-dir", "-r", req_path],
                stdout=log_file, stderr=log_file, timeout=PIP_INSTALL_TIMEOUT
            )
            if result.returncode == 0:
                log_file.write("[setup] Dependencies installed successfully.\n")
            else:
                log_file.write(f"[setup] pip install exited with code {result.returncode}\n")
        except subprocess.TimeoutExpired:
            log_file.write("[setup] pip install timed out.\n")
        except Exception as e:
            log_file.write(f"[setup] pip install failed: {e}\n")
        log_file.flush()
    else:
        log_file.write("\n[setup] No requirements*.txt found, skipping dependency install.\n")
    # Mark as attempted either way, so we don't retry forever on every start.
    open(marker, "w").close()


def build_run_command(main_file, run_dir, port):
    """
    Decide how to launch the app:
    - FastAPI app that does NOT already call uvicorn.run() itself -> launch via `uvicorn module:app`
    - Any script (Flask/plain) -> launch directly, with PORT passed via env so
      scripts using os.environ.get("PORT") still bind correctly.
    - .js files -> node
    """
    if main_file.endswith(".py"):
        full_path = os.path.join(run_dir, main_file)
        content = ""
        try:
        ...
        self_launches_uvicorn = "uvicorn.run(" in content

        if has_fastapi and not self_launches_uvicorn:
            module_name = main_file.rsplit(".py", 1)[0]
            return [sys.executable, "-m", "uvicorn", f"{module_name}:app", "--host", "0.0.0.0", "--port", str(port)]
...
def terminate_process(p):
    """Kill the whole process group so child processes don't linger."""
    if not p:
        return
    try:
...
<head>
    ...
    <style>
        ...
        .error-msg { color: #ff4757; font-size: 12px; margin-top: 10px; }
    </style>
</head>
<body>
...
<body>
    ...
    <div class="card"><h3><i class="fa-solid fa-user-gear"></i> User Management</h3>
        ...
        <div class="user-item"><div class="user-info"><span class="username"><i class="fa-solid fa-circle-user"></i> {{ u_name }}</span>
            <a href="/admin/delete_user/{{ u_name }}" class="btn btn-danger"><i class="fa-solid fa-trash"></i></a>
            </div><div class="project-tags">
    ...
    if request.method == "POST":
        ...
        if l_type == "admin":
            ...
            if not username or not pw:
                ...
                # New user: the password they chose becomes their password.
                db["users"][username] = pw
                save_db(db)
                session['is_admin'], session['username'] = False, username
                return redirect(url_for("index"))
            elif pw == db["users"].get(username):
                session['is_admin'], session['username'] = False, username
                return redirect(url_for("index"))
            else:
...
# ---------------------------------------------------------------------------
# APP LIFECYCLE: run / stop / restart / delete
# ---------------------------------------------------------------------------
...
def run(name):
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        main_file, run_dir = find_main_file(extract_dir)
        log_path = os.path.join(app_dir, "logs.txt")
        log_file = open(log_path, "a")

        if not main_file:
            log_file.write("\n[error] No entry file found (main.py, bot.py, app.py, index.js, server.js).\n")
            log_file.close()
            return redirect(url_for("index"))

        port = find_free_port()
        install_requirements_if_needed(app_dir, run_dir, extract_dir, log_file)

        cmd = build_run_command(main_file, run_dir, port)
        env = os.environ.copy()
        env["PORT"] = str(port)

        log_file.write(f"\n[run] Starting: {' '.join(cmd)} (PORT={port})\n")
        log_file.flush()

        popen_kwargs = dict(cwd=run_dir, stdout=log_file, stderr=log_file, text=True, env=env)
        ...
        try:
            ...
            app_ports[(user_name, name)] = port

            db = load_db()
            db["start_times"][f"{user_name}_{name}"] = int(time.time() * 1000)
            save_db(db)
        except Exception as e:
            log_file.write(f"[error] Failed to start process: {e}\n")
        finally:
            log_file.close()
...
def stop(name):
    ...
    app_ports.pop((user_name, name), None)
    db = load_db()
...
def get_log(name):
    if 'username' not in session:
        return jsonify({"log": "", "status": "OFFLINE", "start_time": 0, "port": None})
    user_name = session['username']
    app_dir = os.path.join(UPLOAD_FOLDER, user_name, name)
    log_path = os.path.join(app_dir, "logs.txt")
    log_content = ""
    ...
    port = app_ports.get((user_name, name))
    return jsonify({
        "log": log_content,
...
def upload():
    ...
    elif lower_name.endswith((".py", ".js", ".txt", ".json", ".env")):
        # Single-file project: wrap it directly into its own extracted/ folder.
        app_name = filename.rsplit('.', 1)[0]
        user_dir = os.path.join(UPLOAD_FOLDER, user_name, app_name)
        extract_dir = os.path.join(user_dir, "extracted")
        os.makedirs(extract_dir, exist_ok=True)
        file.save(os.path.join(extract_dir, filename))
...
def download_file(name):
    ...
    base = os.path.join(UPLOAD_FOLDER, user_name, name, "extracted")
...
def read_content():
    ...
    base = os.path.join(UPLOAD_FOLDER, session['username'], data.get('project', ''), "extracted")
    path = safe_join(base, data.get('filename', ''))
    if path and os.path.exists(path):
...
def save_content():
    ...
    path = safe_join(base, data.get('filename', ''))
    if not path:
        return jsonify({"status": "invalid path"}), 400
...
def create_file_api():
    ...
    data = request.json or {}
    base = os.path.join(UPLOAD_FOLDER, session['username'], data.get('project', ''), "extracted")
    rel_path = data.get('filename', '').strip().lstrip("/")
...
if __name__ == "__main__":
    # Local testing only. On Render, gunicorn runs this via Procfile.
    app.run(host="0.0.0.0", port=int(os.environ.get("PORT", 3522)), debug=False)
