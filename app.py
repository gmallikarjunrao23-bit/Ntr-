...
import os, zipfile, subprocess, shutil, json, time, io, socket, sys
...
def find_requirements(run_dir, extract_dir):
    # Prefer exact requirements.txt, then fall back to any requirements*.txt
    search_dirs = [run_dir, extract_dir]
    for d in search_dirs:
        exact = os.path.join(d, "requirements.txt")
    ...
    if req_path:
        ...
        try:
            subprocess.run(
                ...
                stdout=log_file, stderr=log_file, timeout=300
            )
...
def build_run_command(main_file, run_dir, port):
    """Decide how to launch the app: uvicorn for FastAPI, else plain python/node."""
...
def create_file_api():
    data = request.json
    rel_path = data['filename'].strip().lstrip("/")
    base = os.path.join(UPLOAD_FOLDER, session['username'], data['project'], "extracted")
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        ...
        if main_file:
            ...
            processes[(user_name, name)] = subprocess.Popen(
                cmd, cwd=run_dir, stdout=log_file, stderr=log_file, text=True, env=env
            )
...
def get_log(name):
    ...
    if os.path.exists(log_path):
        with open(log_path, "r") as f: log_content = f.read()[-3000:]
    p = processes.get((user_name, name))
    db = load_db()
...
def download_file(name):
    ...
    path = os.path.join(UPLOAD_FOLDER, user_name, name, "extracted", filename)
    if os.path.exists(path):
